...
관리자 화면은 제출용 로컬 대시보드입니다. Docker 실행 시 `WEB_AUTO_LOGIN=true`가 기본으로 적용되어, 교수자가 저장소를 내려받아 실행해도 별도 회원가입이나 비밀번호 입력 없이 로컬 접속에서 바로 사용할 수 있습니다. 실제 비밀번호 로그인이 필요하면 `.env`에서 `WEB_AUTO_LOGIN=false`, `WEB_ADMIN_USERNAME=admin`, `WEB_ADMIN_PASSWORD=원하는_비밀번호`를 설정합니다.
