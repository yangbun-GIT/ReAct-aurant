...
NON_BEVERAGE_CAFE_TERMS = ["스터디카페", "스터디룸", "공간대여", "서비스,산업", "독서실", "고시원"]
BEVERAGE_CAFE_TERMS = ["카페", "커피", "커피전문점", "라떼", "찻집", "디저트", "브런치", "음료"]
...
def _food_query_terms(food_query: str | None) -> list[str]:
    ...
    if query in SPECIFIC_FOOD_SYNONYMS:
        return _unique_strings([query, *SPECIFIC_FOOD_SYNONYMS[query]])
    strict_term_groups = {
        "술집": ["술집", "주점", "포차", "호프", "펍", "이자카야", "맥주", "소주", "와인바", "칵테일바"],
        "바": ["바", "와인바", "칵테일바", "펍"],
        "혼술": ["혼술", "술집", "주점"],
        "한잔": ["한잔", "술집", "주점"],
        "술자리": ["술자리", "술집", "주점"],
        ...
        "칵테일": ["칵테일", "칵테일바", "바"],
        "칵테일바": ["칵테일바", "칵테일", "바"],
        "빵집": ["빵집", "베이커리", "제과", "제빵", "바게트", "크루아상", "소금빵", "케이크"],
        "베이커리": ["베이커리", "빵집", "제과", "제빵", "바게트", "크루아상", "소금빵", "케이크"],
        "빵": ["빵", "빵집", "베이커리", "제과", "제빵"],
        "카페": BEVERAGE_CAFE_TERMS,
        "디저트카페": ["디저트카페", "디저트 카페", "빙수", "아이스크림", "케이크"],
        "디저트 카페": ["디저트 카페", "디저트카페", "빙수", "아이스크림", "케이크"],
    }
    if query in strict_term_groups:
        return _unique_strings([query, *strict_term_groups[query]])
...
def _matches_food_query(restaurant: dict[str, Any], food_query: str | None) -> bool:
    ...
    if requested == "카페":
        if any(term in blob for term in NON_BEVERAGE_CAFE_TERMS):
            return False
        return any(term in blob for term in BEVERAGE_CAFE_TERMS)
    if requested in {"막걸리", "전집", "파전", "해물파전"}:
        return _has_traditional_drinking_signal(blob)
    if requested == "술집":
        return _has_bar_place_signal(blob)
    return any(term == cuisine or _contains_food_match_term(blob, term) for term in terms)


def _contains_food_match_term(blob: str, term: str) -> bool:
    if term == "커리":
